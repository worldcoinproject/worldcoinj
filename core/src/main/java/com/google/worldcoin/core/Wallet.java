...
import com.google.worldcoin.script.ScriptChunk;
import com.google.worldcoin.store.UnreadableWalletException;
import com.google.worldcoin.store.WalletProtobufSerializer;
import com.google.worldcoin.utils.ListenerRegistration;
import com.google.worldcoin.utils.Threading;
import com.google.worldcoin.wallet.*;
import com.google.worldcoin.wallet.WalletTransaction.Pool;
...
import static com.google.worldcoin.core.Utils.bitcoinValueToFriendlyString;
import static com.google.worldcoin.core.Utils.bitcoinValueToPlainString;
 ...
 * {@link Wallet#autosaveToFile(java.io.File, long, java.util.concurrent.TimeUnit, com.google.worldcoin.wallet.WalletFiles.Listener)}
...
public class Wallet implements Serializable, BlockChainListener, PeerFilterProvider {
    ...
    public void receivePending(Transaction tx, @Nullable List<Transaction> dependencies, boolean overrideIsRelevant) throws VerificationException {
        ...
        try {
            ...
            if (log.isInfoEnabled()) {
                log.info(String.format("Received a pending transaction %s that spends %s WDC from our own wallet," +
                        " and sends us %s WDC", tx.getHashAsString(), Utils.bitcoinValueToFriendlyString(valueSentFromMe),
    ...
    private void receive(Transaction tx, StoredBlock block, BlockChain.NewBlockType blockType,
        ...
        log.info("Received tx{} for {} WDC: {} [{}] in block {}", sideChain ? " on a side chain" : "",
    ...
    public static class SendRequest {
         ...
         * (its value is ignored and set to {@link com.google.worldcoin.core.Wallet#getBalance()} - the fees required
         ...
         *
         * <p>Once {@link Wallet#completeTx(com.google.worldcoin.core.Wallet.SendRequest)} is called, this is set to the
        ...
        /**
         * If not null, the {@link com.google.worldcoin.wallet.CoinSelector} to use instead of the wallets default. Coin selectors are
    ...
    public void completeTx(SendRequest req) throws InsufficientMoneyException {
        ...
        try {
            ...
            if (req.ensureMinRequiredFee && !req.emptyWallet) { // min fee checking is handled later for emptyWallet
                for (TransactionOutput output : req.tx.getOutputs())
                    if (output.getValue().compareTo(Utils.CENT) < 0) { //TXOut lower than 1 DOGE have a 2 DOGE fee!
...
//                            throw new IllegalArgumentException("Tried to send dust with ensureMinRequiredFee set - no way to complete this");
     ...
     * If {@link Wallet#autosaveToFile(java.io.File, long, java.util.concurrent.TimeUnit, com.google.worldcoin.wallet.WalletFiles.Listener)}
    ...
    public String toString(boolean includePrivateKeys, boolean includeTransactions, boolean includeExtensions,
        ...
        try {
            ...
            builder.append(String.format("Wallet containing %s WDC in:%n",
     ...
     * of {@link com.google.worldcoin.core.ECKey#getCreationTimeSeconds()}. This can return zero if at least one key does
    ...
    /**
     * Convenience wrapper around {@link Wallet#encrypt(com.google.worldcoin.crypto.KeyCrypter,
     ...
     * {@link com.google.worldcoin.crypto.KeyCrypterScrypt}.
     *
        ...
        try {
            ...
            for (ECKey key : keychain) {
                if (key.isEncrypted()) {
                    ...
                    // (all worldcoin controlled by that private key is lost forever).
    ...
    /**
     * <p>Convenience wrapper around {@link Wallet#addNewEncryptedKey(com.google.worldcoin.crypto.KeyCrypter,
     ...
     * you can use {@link com.google.worldcoin.utils.Threading#waitForUserCode()} to block until the future had a
    ...
    private class FeeCalculation {
        ...
        public FeeCalculation(SendRequest req, BigInteger value, List<TransactionInput> originalInputs,
            ...
            // 1) No effect
            // 2) Causes increase in fee (change < 1 CENT)
     ...
     */
    public void setTransactionBroadcaster(@Nullable com.google.worldcoin.core.TransactionBroadcaster broadcaster) {
