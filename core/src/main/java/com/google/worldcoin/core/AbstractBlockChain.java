 ...
 * the lightweight SPV mode, a {@link com.google.worldcoin.store.SPVBlockStore} is the right choice.</p>
 *
 ...
 * {@link com.google.worldcoin.store.H2FullPrunedBlockStore} implements full verification, which is equivalent to the
...
public abstract class AbstractBlockChain {
    ...
    // February 16th 2012
    private static final Date testnetDiffDate = new Date(1329264000000L); // TODO move somewhere useful?
    ...
    private void checkDifficultyTransitions(StoredBlock storedPrev, Block nextBlock) throws BlockStoreException, VerificationException {
        ...
        boolean newDiffBlock = storedPrev.getHeight()+1 >= 600000;
        
        // Is this supposed to be a difficulty transition point?
        if ((storedPrev.getHeight() + 1) % params.getInterval(newDiffBlock) != 0) {
        ...
        int goBack = params.getInterval(newDiffBlock) - 1;
        if (cursor.getHeight()+1 != params.getInterval(newDiffBlock))
            goBack = params.getInterval(newDiffBlock);
        ...
        final int targetTimespan = params.getTargetTimespan(newDiffBlock);
        // Limit the adjustment step.
        if (newDiffBlock)
        {
            if (timespan < (targetTimespan - (targetTimespan/10)) ) timespan = (targetTimespan - (targetTimespan/10));
            if (timespan > (targetTimespan + (targetTimespan/10)) ) timespan = (targetTimespan + (targetTimespan/10));
        }
        else
        {
    ...
    private void checkTestnetDifficulty(StoredBlock storedPrev, Block prev, Block next) throws VerificationException, BlockStoreException {
        ...
        // There is an integer underflow bug in worldcoin-qt that means mindiff blocks are accepted when time
        // goes backwards.
        if (timeDelta >= 0 && timeDelta <= NetworkParameters.TARGET_SPACING * 2) {
            ...
            while (!cursor.getHeader().equals(params.getGenesisBlock()) &&
                   cursor.getHeight() % params.getInterval(true) != 0 &&
...
//            long estimated = (headTime * 1000) + (1000L * 60L * 10L * offset); //Wrong for WDC
            long estimated = (headTime * 1000) + (1000L * 30L * 1L * offset); // 2 blocks per minute
