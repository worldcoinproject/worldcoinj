...
import com.google.worldcoin.net.ClientConnectionManager;
import com.google.worldcoin.net.NioClientManager;
import com.google.worldcoin.net.discovery.PeerDiscovery;
import com.google.worldcoin.net.discovery.PeerDiscoveryException;
import com.google.worldcoin.params.MainNetParams;
import com.google.worldcoin.script.Script;
import com.google.worldcoin.utils.ExponentialBackoff;
...
public class PeerGroup extends AbstractExecutionThreadService implements TransactionBroadcaster {
    ...
    @GuardedBy("lock") private long fastCatchupTimeSecs;  // How is this computed?
    private final CopyOnWriteArrayList<Wallet> wallets;
    private final CopyOnWriteArrayList<PeerFilterProvider> peerFilterProviders;

    public static PeerGroup uglyQuickHack;
     ...
     */
    public static final double DEFAULT_BLOOM_FILTER_FP_RATE = 0.0002; // 0.0005
    ...
    public static final int DEFAULT_CONNECT_TIMEOUT_MILLIS = 1000*5;
    ...
    public void addAddress(PeerAddress peerAddress) {
        int newMax;
        // Don't add if already in inactive or active
        if(backoffMap.containsKey(peerAddress)==false && peers.contains(peerAddress)==false){
        lock.lock();
        try {
  ...
  //              System.currentTimeMillis() - start, addressSet.size());
    ...
    public void recalculateFastCatchupAndFilter(boolean forceFilterUpdate) {
        ...
        try {
            ...
            log.info("Earliers key time: " + earliestKeyTimeSecs);
     ...
     *         Use {@link com.google.worldcoin.core.Peer#getConnectionOpenFuture()} if you
    ...
    public void setFastCatchupTimeSecs(long secondsSinceEpoch) {
        lock.lock();
        log.info("Setting fast catchup time: {}", secondsSinceEpoch);
        try {
            ...
            if (downloadPeer != null) {
       //     	log.info("Setting fast download with bloom: {}", bloomFilter);
     ...
     * peers. By using this with {@link com.google.worldcoin.core.PeerGroup#getMaxConnections()} you can wait until the
     ...
     * {@link com.google.worldcoin.core.PeerGroup#getMaxConnections()} returns is used.
     */
    ...
    /**
     * See {@link com.google.worldcoin.core.PeerGroup#getMinBroadcastConnections()}.
     */
     ...
     * times are available via {@link com.google.worldcoin.core.Peer#getLastPingTime()} but it increases load on the
     ...
     * using {@link com.google.worldcoin.core.Peer#ping()}.
     */
