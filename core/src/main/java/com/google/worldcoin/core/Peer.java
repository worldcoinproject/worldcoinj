 ...
 * {@link com.google.worldcoin.net.AbstractTimeoutHandler} and timeout is automatically disabled (using
 * {@link com.google.worldcoin.net.AbstractTimeoutHandler#setTimeoutEnabled(boolean)}) once the version
...
public class Peer extends PeerSocketHandler {
    ...
    private static class GetDataRequest {
        ...
        // a nasty hack that relies on the fact that worldcoin-qt is single threaded and processes messages in order.
    ...
    // fork.
    private final long MIN_PEER_BLOCK_HEIGHT = 1300000;
     ...
     * {@link com.google.worldcoin.net.NioClientManager#openConnection(java.net.SocketAddress, com.google.worldcoin.net.StreamParser)}
     * or
     * {@link com.google.worldcoin.net.NioClient#NioClient(java.net.SocketAddress, com.google.worldcoin.net.StreamParser, int)}.</p>
     *
...
//        log.info("message "+m);
        if (m instanceof NotFoundMessage) {
        	...
        	processCheckpointMessage((CheckpointMessage)m);
    ...
    /** TODO warn & reset chain if we're on wrong fork */
    private void processCheckpointMessage(CheckpointMessage m) throws ProtocolException {
    }
    ...
    //    log.info("header");
        ...
        try {
            ...
            for (int i = 0; i < m.getBlockHeaders().size(); i++) {
                ...
                boolean reachedTop = blockChain.getBestChainHeight() >= vPeerVersionMessage.bestHeight;// Pretty fast so far
//                if(this.downloadBlockBodies==false){
  //               log.info("Catchup: {}  header time:{}",fastCatchupTimeSecs,header.getTimeSeconds());
    //             log.info(" chain height:{}  best height {}",blockChain.getBestChainHeight(),vPeerVersionMessage.bestHeight);
      //          }
             ...
             //       	log.info("added #"+i+" header block:"+header);
    ...
    public void setDownloadParameters(long secondsSinceEpoch, boolean useFilteredBlocks) {
      ...
      //          log.info("Peer setDownloadParameters set download Bodes because epoch =0");
     ...
     //       log.info("useFilteredblocks {}",useFilteredBlocks);
        } finally {
            lock.unlock();
        }
    }
  ...
  //      	log.info("Downloading headers for a while till {}", toHash);
     ...
     * Once the pong is received the value returned by {@link com.google.worldcoin.core.Peer#getLastPingTime()} is
     * updated.
    ...
    /**
     * Returns the elapsed time of the last ping/pong cycle. If {@link com.google.worldcoin.core.Peer#ping()} has never
    ...
    /**
     * Returns a moving average of the last N ping/pong cycles. If {@link com.google.worldcoin.core.Peer#ping()} has never
