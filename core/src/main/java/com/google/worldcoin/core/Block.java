...
public class Block extends Message {
    ...
    public BigInteger getBlockInflation(int height) {
        return Utils.toNanoCoins(50, 0).shiftRight(height / params.getSubsidyDecreaseBlockCount()); // TODO totally wrong for WDC
    }
    ...
    public void verifyHeader() throws VerificationException {
        ...
        maybeParseHeader();  // TODO one of these is mega-slow!
        long currentTime = Utils.currentTimeMillis()/1000;
        if (time + ALLOWED_TIME_DRIFT > currentTime) checkProofOfWork(true);  // Only check diff on the last 2 hours of block. Ugly hack to speed things.
