package com.google.worldcoin.tools;
...
public class BuildCheckpoints {
    public static void main(String[] args) throws Exception {
        ...
        final long oneMonthAgo = now - (86400 * 14); //TODO: Make it higher later on (1 day * 14 days = 2 weeks)

        chain.addListener(new AbstractBlockChainListener() {
            @Override
            public void notifyNewBestBlock(StoredBlock block) throws VerificationException {
                int height = block.getHeight();
                if (height % params.getInterval(height+1>=600000) == 0 && block.getHeader().getTimeSeconds() <= oneMonthAgo) {
